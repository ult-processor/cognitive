...
def get_datasets(asset, currency, granularity, datapoints):
    ...
    if not os.path.exists(df_rollout_path):
        ...
        # converting the training set back to csv format.
        df_train.to_csv(df_train_path)
        # converting the testing set back to csv format.
        df_rollout.to_csv(df_rollout_path)
        # read the traning set from csv to a pandas dataframe
        df_train = pd.read_csv(df_train_path) # re-read to avoid indexing issue w/ Ray
        # reading the testset into the dataframe
